...
if (-not (Test-Path (Join-Path $ProjectRoot "software"))) {
    Write-Error "Could not find software/ relative to repository root ($ProjectRoot)."
}

switch ($Command) {
    "up" {
        Write-Host "Starting ROS 2 container in the background..."
        Set-Location (Join-Path $ProjectRoot "software")
        Invoke-Compose up -d
    }
    "down" {
    ...
    default {
        Write-Host "Usage: ./scripts/dev.ps1 [up|down|shell|build|launch|moveit-sim|moveit-real|moveit-test]"
        exit 1
    }
}
