...
| **Assembly** (how parts relate) | **STEP** of the **top-level assembly** (components can be include as separate parts per export options — choose what keeps file size and clarity reasonable). | **Exploded** view as **PDF** or a rendered image for the wiki/docs. **BOM** as **CSV** if the SolidWorks BOM is the source of truth and not already mirrored in `docs/finances/BOM.md`. |
