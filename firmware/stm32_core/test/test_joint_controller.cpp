...
void run_joint_controller_tests(TestContext& test) {
  ...
  for (int joint = 0; joint < robot_arm::kJointCount; ++joint) {
    test.CheckFloatEq(command.target_position_deg[joint], controller.command().target_position_deg[joint], "joint controller should store the latest command");
  }
  ...
  for (int joint = 0; joint < robot_arm::kJointCount; ++joint) {
    test.CheckFloatEq(measured_state.position_deg[joint], controller.measured_state().position_deg[joint], "joint controller should store measured position");
    test.CheckFloatEq(measured_state.velocity_deg_s[joint], controller.measured_state().velocity_deg_s[joint], "joint controller should store measured velocity");
  }

  controller.Step(0.01F);
  for (int joint = 0; joint < robot_arm::kJointCount; ++joint) {
    test.CheckFloatEq(command.target_position_deg[joint], controller.command().target_position_deg[joint], "joint controller step should not clear the command yet");
    test.CheckFloatEq(measured_state.position_deg[joint], controller.measured_state().position_deg[joint], "joint controller step should preserve measured position");
  }
